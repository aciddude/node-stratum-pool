...
var JobManager = module.exports = function JobManager(options) {
    ...
    var blockHasher = (function () {
        switch (options.coin.algorithm) {
            case 'neoscrypt':
                if (options.coin.name === 'trezarcoin') {
                    console.log("Trezarcoin blockHasher!!! \n")
                    return function (d) {
                        var version = d.slice(0, 1)     // Trezarcoin Block_version
                        var keyTwo = d.slice(58, 68)    // Trezarcoin Block_HashingKey1
                        var keyOne = d.slice(36, 46)    // Trezarcoin Block_HashingKey1
                        var _nTime = d.slice(68, 72)    // Trezarcoin Block_nTime
                        var _nBits = d.slice(72, 76)    // Trezarcoin Block_nBits
                        var _nonce = d.slice(76, 80)    // Trezarcoin Block_nonce
                        var _fullMerkle = d.slice(36, 68)   // Trezarcoin Block_merkleRoot
                        var _input112BytesA = [d,  _fullMerkle]   // Entire Block Hex + MerkleRoot Array
                        var _input112Bytes = Buffer.concat(_input112BytesA) // Contact Array
                        var _finalKeyA = [keyTwo , _nTime , _nBits , _nonce , keyOne]   // Key for block hashing Array
                        var _finalKey  = Buffer.concat(_finalKeyA) // Concact Array
                        // console.log("Trezarcoin version:",util.reverseHex(version))
                        // console.log("Trezarcoin keyOne:",util.reverseHex(keyOne))
                        // console.log("Trezarcoin keyTwo:",util.reverseHex(keyTwo))
                        // console.log("Trezarcoin nTime:",parseInt(util.reverseHex(_nTime), 16))
                        // console.log("Trezarcoin nBits:",util.reverseHex(_nBits))
                        // console.log("Trezarcoin nonce:",parseInt(util.reverseHex(_nonce), 16))
                        // console.log("Trezarcoin fullMerkle:",util.reverseHex(_fullMerkle))
                        // console.log("Trezarcoin Input 112 Bytes:",_input112Bytes.toString('hex'))
                        // console.log("Trezarcoin Final Key:", _finalKey.toString('hex'))
                        var blake2sBlockHeader = new BLAKE2s(32, _finalKey)
                        blake2sBlockHeader.update(_input112Bytes)
                        console.log("Trezarcoin Block Header !!!", util.reverseHex(blake2sBlockHeader.digest()))
                        return util.reverseBuffer(blake2sBlockHeader.digest(this));
                    };
                }
            case 'scrypt':
    ...
    this.processShare = function (jobId, previousDifficulty, difficulty, extraNonce1, extraNonce2, nTime, nonce, ipAddress, port, workerName, logger) {
        ...
        if (job.target.ge(headerBigNum)) {
            ...
            if (options.coin.algorithm === 'trezarcoin') {
