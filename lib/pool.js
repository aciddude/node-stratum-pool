...
var pool = module.exports = function pool(options, authorizeFn, logger) {
    ...
    function SubmitBlock(blockHex, callback) {
        ...
        if (options.hasSubmitMethod) {
            ...
            rpcArgs = [{'mode': 'proposal', 'data': blockHex}]; // TODO: is mode submit even correct? No it isn't
        }
        ...
        _this.daemon.cmd(rpcCommand,
            ...
            function (results) {
                ...
                logger.warn('This was the block hex %s', blockHex)
                callback();
            }
        );
    ...
    function GetBlockTemplate(callback) {
        _this.daemon.cmd('getblocktemplate',
            [{"capabilities": ["coinbasetxn", "workid", "coinbase/append"], "rules": ["csv"]}],
            function (result) {
                if (result.error) {
